...
Adafruit_NeoPixel ledStrip[] = {Adafruit_NeoPixel(LED_COUNT_TORSO_HEAD, LED_PIN_TORSO_HEAD, NEO_GRB + NEO_KHZ800),
                                Adafruit_NeoPixel(LED_COUNT_L_ARM, LED_PIN_L_ARM, NEO_GRB + NEO_KHZ800),
                                Adafruit_NeoPixel(LED_COUNT_R_ARM, LED_PIN_R_ARM, NEO_GRB + NEO_KHZ800),
                                Adafruit_NeoPixel(LED_COUNT_L_LEG, LED_PIN_L_LEG, NEO_GRB + NEO_KHZ800),
                                Adafruit_NeoPixel(LED_COUNT_R_LEG, LED_PIN_R_LEG, NEO_GRB + NEO_KHZ800)
                               };
...
void GN_running(bool trans_am, int fadeSpeed) {
  ...
  if (trans_am == true) {
    suitColor = 65536 / 20;
    rgb = ledStrip[0].ColorHSV(65536 / 6, 255, 150);
    for (int j = 0; j < 8; j++) {
      ledStrip[0].setPixelColor(LED_INDEX[0][j], rgb);
    }
    showAll();
  ...
  for (int i = 25; i < 255 / 2; i++) {
    rgb = ledStrip[1].ColorHSV(suitColor, 255, i);
    for (int j = 0; j < LED_COUNT_R_ARM; j++) {
      delay(1);
      ledStrip[1].setPixelColor(LED_INDEX[1][j], rgb);
      ledStrip[2].setPixelColor(LED_INDEX[1][j], rgb);
      ledStrip[3].setPixelColor(LED_INDEX[2][j], rgb);
      ledStrip[4].setPixelColor(LED_INDEX[2][j], rgb);
    }
    showAll();
    delay(fadeSpeed);
  }
  ...
  for (int i = 255 / 2; i > 25; i--) {
    rgb = ledStrip[0].ColorHSV(suitColor, 255, i);
    for (int j = 0; j < LED_COUNT_R_ARM; j++) {
      ledStrip[1].setPixelColor(LED_INDEX[1][j], rgb);
      ledStrip[2].setPixelColor(LED_INDEX[1][j], rgb);
      ledStrip[3].setPixelColor(LED_INDEX[2][j], rgb);
...
void rainbow1(int wait) {
  for (long firstPixelHue = 0; firstPixelHue < 5 * 65536; firstPixelHue += 256) {
    for (int j = 0; j < NUM_STRIPS; j++) {
      for (int i = 0; i < ledStrip[j].numPixels(); i++) { // For each pixel in strip...
        int pixelHue = firstPixelHue + (i * 65536L / ledStrip[j].numPixels());
        ledStrip[j].setPixelColor(i, ledStrip[j].gamma32(ledStrip[j].ColorHSV(pixelHue)));
      }
